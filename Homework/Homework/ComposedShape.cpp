...
void ComposedShape::optimalFit(const Circle& Circ, char SShape) {
	...
	if (SShape == 'c') {
		...
		double Angle, param;
		int newRad = Circ.getShape();		//To solve the fitting of 1/2 radius ratio circle.
		newRad++;
	...
	if (SShape == 'r') {
		...
		int count1, count2, height, width, length, replacement;
		int s = 1;			//counter
		int rect_x;
		int rect_y;
		double pow1;
...
void ComposedShape::optimalFit(const Triangle& MCTriangle, char SShape) {
...
void ComposedShape::modifySShape(const char SShape) {
	...
	case 'r':
		...
		//RectInTri(WofRect, HofRect, MCTri);
		break;
	case 't':
		cout << "Please enter the lenght of triangle: ";
		cin >> LofTri;

		//TriInTri(LofTri, MCTri);
		break;
	}
